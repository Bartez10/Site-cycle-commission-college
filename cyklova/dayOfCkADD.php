...
<div>
  <table style="width: 100%;">
    ...
    <?php
    $vyhovnaCK = mysqli_query($connection, "SELECT id_vyhovna, title FROM vyhovnack");
    $vyhovnaCK = mysqli_fetch_all($vyhovnaCK);
    foreach ($vyhovnaCK as $vyhovnaC) {
      ?>
       <tr>
         <td><?=$vyhovnaC[0]?></td>
         <td><?=$vyhovnaC[1]?></td>
         <td><a href="updateVyhovnaCK.php?id=<?=$vyhovnaC[0]?>">Змінити</a></td>
         <td><a style="color: red" href="delete-vyhovnack.php?id=<?=$vyhovnaC[0]?>">Видалити</a></td>
      </tr>
    <?php
  }
    ?>
...
<form style="width: 600px; margin-top:50px;" action="add-newCK.php" method="POST" enctype="multipart/form-data" >
