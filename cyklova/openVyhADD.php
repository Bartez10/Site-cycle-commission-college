<?php
  $page_title = 'Внесення інформації про відкриті виховні';
...
<div>
  <table style="width: 100%;">
    ...
    <?php
    $vyhovna = mysqli_query($connection, "SELECT id_vyhovna, title FROM vyhovna");
    $vyhovna = mysqli_fetch_all($vyhovna);
    foreach ($vyhovna as $vyhovn) {
      ?>
       <tr>
         <td><?=$vyhovn[0]?></td>
         <td><?=$vyhovn[1]?></td>
         <td><a href="updateVyhovna.php?id=<?=$vyhovn[0]?>">Змінити</a></td>
         <td><a style="color: red" href="delete-vyhovna.php?id=<?=$vyhovn[0]?>">Видалити</a></td>
      </tr>
    <?php
  }
    ?>
...
<form style="width: 600px; margin-top:50px;" action="add-newVyhovna.php" method="POST" enctype="multipart/form-data" >
