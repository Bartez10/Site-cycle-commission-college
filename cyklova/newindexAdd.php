<?php
  ...
  $page_title = 'Внесення новин на головну';
...
<div>
  <table style="width: 100%;">
    ...
    <?php
    $newindexs = mysqli_query($connection, "SELECT id, title FROM newIndex");
    $newindexs = mysqli_fetch_all($newindexs);
    foreach ($newindexs as $newindex) {
      ?>
       <tr>
         <td><?=$newindex[0]?></td>
         <td><?=$newindex[1]?></td>
         <td><a style="color: red" href="delete-newindex.php?id=<?=$newindex[0]?>">Видалити</a></td>
      </tr>
    <?php
  }
    ?>
...
<form style="width: 600px; margin-top:50px;" action="add-newindex.php" method="POST" enctype="multipart/form-data" >
