<?php
  $page_title = 'Внесення інформації про дипломні';
...
<div>
  <table style="width: 100%;">
    ...
    <?php
    $diploma = mysqli_query($connection, "SELECT id_diploma, tema, student FROM diploma");
    $diploma = mysqli_fetch_all($diploma);
    foreach ($diploma as $diplom) {
      ?>
       <tr>
         <td><?=$diplom[0]?></td>
         <td><?=$diplom[1]?></td>
         <td><?=$diplom[2]?></td>
         <td><a href="updateDiploma.php?id=<?=$diplom[0]?>">Змінити</a></td>
         <td><a style="color: red" href="delete-diploma.php?id=<?=$diplom[0]?>">Видалити</a></td>
      </tr>
    <?php
  }
    ?>
...
<form style="width: 600px; margin-top:50px;" action="add-diploma.php" method="POST" enctype="multipart/form-data" >
...
<div class="input-group mb-3">
  ...
  <textarea name="kerivnyk" type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default"></textarea>
</div>
